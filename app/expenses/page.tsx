...
export default function ExpensesPage() {
  ...
  const handleExportCSV = () => {
    const csvHeader = [t.date, t.description, t.category, t.vendor, t.method, t.amount].join(',');
    
    const csvRows = filteredExpenses.map(expense => {
      const date = format(new Date(expense.date), 'yyyy-MM-dd');
      const desc = `"${expense.description.replace(/"/g, '""')}"`;
      const cat = `"${expense.category.replace(/"/g, '""')}"`;
      const vendor = `"${(expense.vendor || '').replace(/"/g, '""')}"`;
      const method = `"${(expense.payment_method || '').replace(/"/g, '""')}"`;
      const amount = expense.amount;
      return [date, desc, cat, vendor, method, amount].join(',');
    });
  ...
  return (
    <div className="flex-1 space-y-4 p-4 lg:p-8 pt-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="outline" className="gap-2 shadow-sm h-11 rounded-xl text-sm font-semibold border-zinc-200" onClick={handleExportCSV}>
