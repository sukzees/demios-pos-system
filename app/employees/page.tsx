...
import { supabase, EmployeePermissions } from '@/lib/supabase';
...
const TRANSLATIONS = {
  en: {
    ...
    manageAccess: 'Manage menu access and action permissions',
    ...
    pos: 'POS',
    history: 'History',
    itemsCategories: 'Items & Categories',
    inventory: 'Inventory',
    expenses: 'Expenses',
    reports: 'Reports',
    settings: 'Settings',
  ...
  lo: {
    ...
    johnDoe: 'ທ້າວ ສົມຊາຍ',
    permissions: 'ສິດການເຂົ້າເຖິງ',
    manageAccess: 'ຈັດການການເຂົ້າເຖິງເມນູ ແລະ ການກະທຳ',
    menuAccess: 'ການເຂົ້າເຖິງເມນູ',
    actionPermissions: 'ສິດການກະທຳ',
    view: 'ເບິ່ງ',
    create: 'ສ້າງ',
    edit: 'ແກ້ໄຂ',
    delete: 'ລົບ',
    ...
    pos: 'ຂາຍສິນຄ້າ',
    history: 'ປະຫວັດ',
    itemsCategories: 'ສິນຄ້າ ແລະ ປະເພດ',
    inventory: 'ສາງສິນຄ້າ',
    expenses: 'ລາຍຈ່າຍ',
    reports: 'ລາຍງານ',
    settings: 'ຕັ້ງຄ່າ',
  ...
  th: {
    ...
    manageAccess: 'จัดการการเข้าถึงเมนูและการกระทำ',
    menuAccess: 'การเข้าถึงเมนู',
    actionPermissions: 'สิทธิ์การกระทำ',
    view: 'ดู',
    create: 'สร้าง',
    edit: 'แก้ไข',
    delete: 'ลบ',
    ...
    pos: 'ขายหน้าร้าน',
    history: 'ประวัติ',
    itemsCategories: 'สินค้าและหมวดหมู่',
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="sm:max-w-[500px] rounded-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid gap-4 py-4">
                ...
                {/* Permissions Section */}
                <div className="grid gap-3 pt-2 border-t border-zinc-200">
                  <div>
                    <Label className="text-base font-bold">{t.permissions}</Label>
                    <p className="text-xs text-zinc-500 mt-1">{t.manageAccess}</p>
                  </div>

                  {/* Menu Access */}
                  <div className="grid gap-2">
                    <Label className="text-sm font-semibold text-zinc-700">{t.menuAccess}</Label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { key: 'dashboard', label: t.dashboard },
                        { key: 'pos', label: t.pos },
                        { key: 'history', label: t.history },
                        { key: 'items', label: t.itemsCategories },
                        ...
                        <label key={menu.key} className="flex items-center gap-2 p-2 rounded-lg hover:bg-zinc-50 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={newEmployee.permissions.menus[menu.key as keyof typeof newEmployee.permissions.menus] || false}
                            onChange={(e) => {
                              setNewEmployee({
                                ...newEmployee,
                                permissions: {
                          ...
                          />
                          <span className="text-sm text-zinc-700">{menu.label}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Action Permissions */}
                  <div className="grid gap-2">
                    <Label className="text-sm font-semibold text-zinc-700">{t.actionPermissions}</Label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <label key={action.key} className="flex items-center gap-2 p-2 rounded-lg hover:bg-zinc-50 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={newEmployee.permissions.actions[action.key as keyof typeof newEmployee.permissions.actions] || false}
                            onChange={(e) => {
                              setNewEmployee({
                                ...newEmployee,
                                permissions: {
                                  ...newEmployee.permissions,
                                  actions: {
                                    ...newEmployee.permissions.actions,
                            ...
                            className="h-4 w-4 rounded border-zinc-300 text-blue-600 focus:ring-blue-500"
                          />
                          <span className="text-sm text-zinc-700">{action.label}</span>
                        </label>
                      ))}
                    </div>
                  </div>
